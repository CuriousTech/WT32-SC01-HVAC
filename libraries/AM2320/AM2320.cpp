/**
 Modified by CuriousTech for stability
...
bool AM2320::measure(int16_t& temp, int16_t& rh)
{
    ...
    rh = (int16_t)((_buf[2] << 8) | _buf[3]); // big endien
    temp = (int16_t)((_buf[4] << 8) | _buf[5]);
    return true;
}
...
bool AM2320::_read_registers(int startAddress, int numByte) 
{
    ...
    uint16_t r = Wire.endTransmission(true);
    if(r != 0){
        code = r;
        return false;           // sensor not ready
    }
    delay(3);                    // as specified in datasheet
...
uint16_t AM2320::crc16(uint8_t *byte, int numByte) {
